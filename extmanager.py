...
def build_ext_with_numpy_include_dir_factory(supercls):
    ...
    class BuildExtWithNumpyIncludeDir(supercls):
        ...
        def build_extensions(self):
            inc = pkg_resources.resource_filename('numpy', 'core/include')

            for ext in self.extensions:
                if hasattr(ext, 'include_dirs') and inc not in ext.include_dirs:
...
def build_ext_allowing_failure_factory(supercls):
    ...
    class BuildExtAllowingFailure(supercls):
        """Allows for extensions to be optional and to fail to install"""
        def build_extensions(self):
            try:
                supercls.build_extensions(self)
...
def s_dist_invoking_cython_factory(supercls):
    ...
    class SDistInvokingCython(supercls):
        """Ensures Cython has compiled all pyx files to .c/.cpp"""
...
class CythonExtension(object):
    ...
    def as_cythonizable_extension(self):
        return Extension(self.name, [self.pyx_path] + self.sources,
                         language=self.language)


def check_extensions_are_valid(extensions):
    missing_exts = list(filter(lambda e: not op.exists(e.pyx_path),
                               extensions))
...
def ext_setup_kwargs(extension_specs, cmdclass=None, min_cython_version=None,
                     failure_permitted=False):

    cmdclass = cmdclass if cmdclass is not None else {}
    sdist_base_cls = cmdclass.get('sdist', default_sdist)
    ...
    if cython_required:
        if not cython_installed:
            ...
            message = ('Cython will run as the following extensions are '
                       'missing:\n{}'.format('\n'.join([p.cythonized_path
                                             for p in ext_needing_cython])))
            print(message)

            cython_numpy_build_ext = build_ext_with_numpy_include_dir_factory(
                    cython_build_ext_factory()
                    )
            # prepare a build_ext that will just compile the pyx files to
            # .c/.cpp files and stop. We will use this for sdists only.
            cythonize_and_stop_build_ext = build_ext_without_compile_factory(
                    cython_numpy_build_ext
                )
            ...
            # to build an sdist.
            cmdclass['sdist'] = s_dist_invoking_cython_factory(sdist_base_cls)

            # make sure the ext modules are ready for processing by the
            # cythonize compiler.
            ext_modules = [e.as_cythonizable_extension() for e in extensions]

            # If we build_ext it will need to be the full cython build ext with
            # the numpy include dir.
        ...
        # 'standard' fashion - no customisation needed.

        # Just use the default build ext and add the numpy
        # include dir
        build_ext = build_ext_with_numpy_include_dir_factory(
